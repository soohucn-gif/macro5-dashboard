...
| # | 模块 | 频率 | 数据源 | 历史起点 |
|---|---|---|---|---|
| ① | 利率期限结构：实际 vs 名义 5/10/30 年 | 日频 | FRED `DFII5/10/30`（实际）· `DGS5/10/30`（名义） | 5/10年 2003-01；30年 2010-02 |
...
**① 实际与名义必须并排看，而且不能只看 10 年。** `DFII5/10/30` 是 TIPS 实际利率，
`DGS5/10/30` 是同期限名义国债利率。看板里**同期限同色**：实线实际、虚线名义，
**两条线之间的竖直间距就是该期限的盈亏平衡通胀**。

这样一眼就能判断利率上行是哪一种：
- 名义在涨、间距不变 → 涨的是**实际利率**，是钱本身的价格，加息帮不上忙；
- 间距跟着张开 → 是**通胀预期**在推，这种美联储可以用加息应对。

三个期限一起看才知道是曲线整体抬升还是只有长端在动 —— **长端实际利率才是压估值的那一根**。
同表还存了盈亏平衡通胀，并算出 `implied_real = DGS − T*YIE` 作交叉验算。
...
- 每日两枪：主枪 22:12 UTC、补枪 00:12 UTC —— 抓取 → 重建看板 → 飞书推送 → 提交推送。
  GitHub 的定时触发天天迟到：2026-09 实测主枪约 00:00–00:50 UTC（北京 08:00–08:50）才开跑，
  补枪约 04:30–05:00 UTC。飞书推送实际在北京 8 点多到
- 每月 2 号 12:23 UTC：额外生成上月月度快照
- 也可在 Actions 页手动触发，勾选「全量回补」「月度快照」或「强制推送」

单一数据源失败不会中断整轮：能抓到的照常落库提交，最后一步再把失败源标红，
GitHub 会就失败的 workflow 发通知。飞书推送失败同理，不会挡住当天数据的提交。
